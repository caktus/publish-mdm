...
Device management with Publish MDM is optional but enables powerful features like remote configuration, security policies, and automated form deployment. If you just want to publish forms without managing devices, check out the :doc:`Quickstart Guide (Form Publishing) <quickstart>` instead.

Step 1: Create a New Organization
---------------------------------

All organizations in Publish MDM are independent, with their own settings and devices. Due to Android Enterprise enrollment requirements, you must create the organization with a **dedicated Gmail account** (not a Google Workspace account). To begin:
...
1. **Create a dedicated Gmail account** for your organization (e.g., ``myorg.myuser.publishmdm@gmail.com``).
   ...
   - You may also use a personal Gmail account, but it cannot be a Google Workspace (business) account. You can still invite your work Google accounts as team members later.
...
2. On the initial setup screen ("Hi there" or "Welcome", depending on your Android version), **rapidly** tap the center or bottom of the screen **seven times or more** to reveal the hidden QR code scanner. If you don't see the scanner after several taps, try again in a different area.
...
2. Open the link and follow the prompts to create a work profile. Once complete, the work profile is enrolled and will receive app assignments and policies.
3. For more details on the work profile user experience, please refer to `What is an Android Work Profile? <https://support.google.com/work/android/answer/6191949>`_
4. Refresh the **Devices** page to confirm your device appears in the list. The device will be configured according to the assigned **Policy**. Note that for this option, the serial number that appears is a random identifier assigned to the work profile, not the actual serial number of the device.
...
Congratulations! You've successfully set up Publish MDM and enrolled your first device. Here are some useful next steps:

- **Learn about Device Management:** Visit the :doc:`Mobile Device Management <mdm>` guide to understand policies, device security, and advanced MDM concepts.
