...
description: >
  ...
  and producing an actionable report. Modelled after the blog post at
  https://medium.com/@hungry.soul/building-a-secure-code-review-agent-c8b2231ac6ed.
...
```bash
git add ${input:reportFile:security-review-report.md}
git commit -m "security: automated code review report"
git push -u origin HEAD
```

Open a PR against the branch specified in the user's prompt. The PR title should be:

```
security: automated code review — recon, hunt, confirm, report
```

The PR body should include a summary table pulled from `security-review-state.json`:

```markdown
## Security Code Review

This PR was generated by the automated `security-tester` workflow across four phases.

### Summary

Total raw findings: N → Confirmed: N → Denied: N
...
<list confirmed_findings sorted by severity, one line each>

See [security-review-report.md](security-review-report.md) for full details.
```
