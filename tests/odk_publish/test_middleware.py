...
from apps.odk_publish.middleware import OrganizationMiddleware, ODKProjectMiddleware
...
class TestCustomMiddleware:
    ...
    def test_valid_organization_slug_and_project_id(self, organizations, projects, rf, middleware):
        ...
        url = reverse("odk_publish:app-user-list", args=url_args)
        request = self.get_request(rf, url)
        ...
        assert [(i.label, i.path) for i in request.odk_project_tabs] == [
            ("Form Templates", reverse("odk_publish:form-template-list", args=url_args)),
            ("App Users", url),
        ]

    def test_valid_organization_slug(self, organizations, rf, middleware):
        """Tests the middleware with a valid URL that includes an organization slug but no project ID."""
        organization = organizations[0]
        url = reverse("odk_publish:server-sync", args=[organization.slug])
        request = self.get_request(rf, url)
        view_func, view_args, view_kwargs = request.resolver_match
    ...
    def test_homepage(self, organizations, rf, middleware):
    ...
    def test_homepage_no_organizations(self, rf, middleware):
        ...
        assert request.odk_project is None
        assert request.odk_projects.count() == 0
        assert request.odk_project_tabs == []

    def test_nonexistent_organization(self, rf, middleware):
        """Test the middleware with a URL with an organization slug that does not exist."""
        url = reverse("odk_publish:app-user-list", args=["does-not-exist", 999])
        request = self.get_request(rf, url)
        view_func, view_args, view_kwargs = request.resolver_match
    ...
    def test_project_in_different_organization(self, organizations, projects, rf, middleware):
        ...
        """
        url = reverse("odk_publish:app-user-list", args=[organizations[0].slug, projects[-1].pk])
        request = self.get_request(rf, url)
        view_func, view_args, view_kwargs = request.resolver_match
