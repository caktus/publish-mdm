...
from apps.odk_publish.etl.excel import (
    get_header,
    get_cell_by_value,
...
def test_find_name_column(survey_sheet, column_name, expected_column_index):
    cell = get_header(sheet=survey_sheet, column_name=column_name)
    assert cell.column == expected_column_index


def test_find_name_column_not_found(survey_sheet):
    cell = get_header(sheet=survey_sheet, column_name="not a column")
    assert cell is None


def test_find_cell_in_column(survey_sheet):
...
def test_find_cell_in_column_not_found(survey_sheet):
    name_header = get_header(sheet=survey_sheet, column_name="name")
    cell = get_cell_by_value(column_header=name_header, value="not a value")
    assert cell is None


def test_set_template_variable(survey_sheet):
    variables = [
