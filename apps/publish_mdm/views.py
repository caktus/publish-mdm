...
def policy_add(request, organization_slug):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect("publish_mdm:policy-edit", organization_slug, policy.pk)
    else:
        form = PolicyNameForm()
    context = {
        "form": form,
...
def policy_edit(request, organization_slug, policy_id):
    ...
    context = {
        ...
        "breadcrumbs": Breadcrumbs.from_items(
            ...
            items=[
                ("Policies", "policy-list"),
                (policy.name, "policy-edit", [policy.pk]),
            ],
        ),
    }
...
def policy_save_name(request, organization_slug, policy_id):
    ...
    if form.is_valid():
        ...
        return render(
            request,
            "publish_mdm/partials/policy_name_section.html",
            {
                "policy": policy,
                "name_form": PolicyNameForm(instance=policy),
                "saved": True,
...
def policy_save_odk_package(request, organization_slug, policy_id):
    ...
    if form.is_valid():
        ...
        return render(
            request,
            "publish_mdm/partials/policy_odk_package.html",
            {
                "policy": policy,
                "odk_package_form": OdkCollectPackageForm(instance=policy),
...
def policy_add_application(request, organization_slug, policy_id):
    ...
    if form.is_valid():
        ...
        applications = policy.applications.order_by("order", "pk")
        app_forms = [
            (a, PolicyApplicationForm(instance=a, prefix=f"app_{a.pk}")) for a in applications
        ]
        return render(
            request,
            "publish_mdm/partials/policy_applications_section.html",
            {
                "policy": policy,
                "app_forms": app_forms,
                "add_app_form": PolicyApplicationAddForm(),
...
def policy_save_application(request, organization_slug, policy_id, app_id):
    ...
    if form.is_valid():
        ...
        return render(
            request,
            "publish_mdm/partials/policy_app_row.html",
            {
                "policy": policy,
                "app": app,
...
def policy_delete_application(request, organization_slug, policy_id, app_id):
    ...
    # Return updated applications section
    applications = policy.applications.order_by("order", "pk")
    app_forms = [(a, PolicyApplicationForm(instance=a, prefix=f"app_{a.pk}")) for a in applications]
    return render(
        request,
        "publish_mdm/partials/policy_applications_section.html",
        {
...
def policy_save_password(request, organization_slug, policy_id):
    ...
    if form.is_valid():
        ...
        return render(
            request,
            "publish_mdm/partials/policy_password_section.html",
            {
                "policy": policy,
                "password_form": PasswordPolicyForm(instance=policy),
                "saved": True,
...
def policy_save_vpn(request, organization_slug, policy_id):
    ...
    if form.is_valid():
        ...
        return render(
            request,
            "publish_mdm/partials/policy_vpn_section.html",
            {
                "policy": policy,
                "vpn_form": VPNForm(instance=policy),
                "saved": True,
...
def policy_save_developer(request, organization_slug, policy_id):
    ...
    if form.is_valid():
        ...
        return render(
            request,
            "publish_mdm/partials/policy_developer_section.html",
            {
                "policy": policy,
                "developer_form": DeveloperSettingsForm(instance=policy),
                "saved": True,
